 ...
 * @param options - Global options (seed, ignoreExamples, maxDepth)
 * @returns A MockClient object
 */
export function createMockClient(spec: SpecInput, options: GlobalOptions = {}): MockClient {
  const { seed, ignoreExamples: globalIgnoreExamples = false, maxDepth = 3 } = options;
  ...
  return {
    async data(callOptions: DataOptions = {}): Promise<Map<string, Map<number, unknown>>> {
      ...
      const { operations: operationsFilter, statusCodes: globalStatusCodes, ignoreExamples: callIgnoreExamples } = callOptions;
      ...
      for (const { operationId, operation } of allOperations) {
        ...
        const codesToGenerate = selectStatusCodes(operation, operationOptions, globalStatusCodes);
        ...
        for (const statusCode of codesToGenerate) {
          ...
          // Generate data using the full priority chain
